...
"""
Header for utils_Task12_DM_main.py
--------------------
This is a copy of the original utils.py code provided.
This file is generated due to a few changes done to the code: 
Used 'sys.exit' to exit the program; this is preferred in scripts for Task 2 of Project A, because 'exit()' is not guaranteed to work in all contexts (like in Jupyter notebooks).
...
from networks_Task2 import MLP, ConvNet, LeNet, AlexNet, AlexNetBN, VGG11, VGG11BN, ResNet18, ResNet18BN_AP, ResNet18BN
...
def get_network(model, channel, num_classes, im_size=(32, 32)):
    ...
    #     # Adjust the first convolutional layer for the input channel size (e.g., RGB)
...
def match_loss(gw_syn, gw_real, args):
    ...
    if args.dis_metric == 'ours':
        ...
        sys.exit('unknown distance function: %s'%args.dis_metric)
...
def get_loops(ipc):
    ...
    if ipc == 1:
        ...
        sys.exit('loop hyper-parameters are not defined for %d ipc'%ipc)
...
def get_eval_pool(eval_mode, model, model_eval):
    if eval_mode == 'M': # multiple architectures
        ...
        model_eval_pool = ['ConvNetD1', 'ConvNetD2', 'ConvNetD3', 'ConvNetD4', 'ConvNetD7']
...
def DiffAugment(x, strategy='', seed = -1, param = None):
    ...
    if strategy:
        if param.aug_mode == 'M': # original
            ...
            sys.exit('unknown augmentation mode: %s'%param.aug_mode)
        x = x.contiguous()
    return x
