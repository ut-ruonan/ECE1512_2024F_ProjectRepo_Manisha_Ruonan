...
Header for utils.py
--------------------
This file provides essential utilities for the project.
The `ConvNet` model specifically is defined in `ConvNet_continual`, where a custom 
convolutional neural network has been designed to support continual learning. 

Note:
The `ConvNet` class from `ConvNet_continual` provides the convolutional network architecture 
used for continual learning experiments, while additional models (e.g., MLP, LeNet, AlexNet, 
ResNet18, etc.) are available from the `networks` module for flexible model selection.
"""
...
from networks import MLP, LeNet, AlexNet, AlexNetBN, VGG11, VGG11BN, ResNet18, ResNet18BN_AP, ResNet18BN
